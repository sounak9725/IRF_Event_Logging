...
function checkRateLimit(userId) {
  ...
  const recentRequests = userRequests.filter(
    (time) => now - time < RATE_LIMIT_CONFIG.WINDOW
  );
  if (recentRequests.length >= RATE_LIMIT_CONFIG.MAX_REQUESTS) {
    const oldestRequest = recentRequests[0];
    const timeLeft = Math.ceil(
      (RATE_LIMIT_CONFIG.WINDOW - (now - oldestRequest)) / 1000 / 60
    );
  ...
  if (recentRequests.length > 0) {
    ...
    if (timeSinceLastRequest < RATE_LIMIT_CONFIG.COOLDOWN) {
      const timeLeft = Math.ceil(
        (RATE_LIMIT_CONFIG.COOLDOWN - timeSinceLastRequest) / 1000
      );
...
async function handleError(interaction, error) {
  console.error(
    `[${error.code || ERROR_CODES.UNKNOWN_ERROR}] ${error.message}`,
    error
  );
...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("event_type")
        .setDescription("What type of event was hosted?")
        .setRequired(true)
        .addChoices(
    ...
    .addStringOption((option) =>
      option
        .setName("cohosts")
        .setDescription("Co-host usernames (comma-separated)")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("attendees")
        .setDescription("Attendees usernames (comma-separated)")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
  ...
  run: async (client, interaction) => {
    ...
    try {
      const hasRole = logevent.some((roleId) =>
      ...
      if (!rateLimitCheck.allowed) {
        ...
        throw new LogQuotaError(
          ...
          `Please wait ${rateLimitCheck.timeLeft} ${unit}.`
        );
      }
      ...
      if (!rowifi.success)
        throw new LogQuotaError(
          "Unable to fetch your Roblox username.",
      ...
      let attempts = 0;
      while (
        (await isRowFilled(SPREADSHEET_ID, SHEET_NAME, nextRow)) &&
        attempts++ < 5
      )
        nextRow++;
      ...
      await handleError(
        interaction,
        error instanceof LogQuotaError
          ? error
          : new LogQuotaError(
